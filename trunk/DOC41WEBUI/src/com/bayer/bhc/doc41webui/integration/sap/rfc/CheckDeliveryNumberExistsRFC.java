...
public class CheckDeliveryNumberExistsRFC extends AbstractDoc41RFC<Boolean>{
	...
	public void prepareCall(JCoFunction pFunction, List<?> pInputParms)
			throws SAPException {
		Doc41Log.get().debug(CheckDeliveryNumberExistsRFC.class, null, "prepareCall():ENTRY");
        ...
        if (pFunction != null) {
            if (pInputParms != null) {
                ...
                throw new SAPException(
                        "CheckDeliveryNumberExistsRFC pInputParms list is null", null);
            }
        } else {
            throw new SAPException(
                    "CheckDeliveryNumberExistsRFC pFunction list is null", null);
        }
        Doc41Log.get().debug(CheckDeliveryNumberExistsRFC.class, null, "prepareCall():EXIT");
	}

	@Override
	public List<Boolean> processResult(JCoFunction pFunction)
			throws SAPException {
		Doc41Log.get().debug(CheckDeliveryNumberExistsRFC.class, null, "processResult():ENTRY");
        ...
        Doc41Log.get().debug(CheckDeliveryNumberExistsRFC.class, null, "processResult():EXIT");
        return mResult;
	}
