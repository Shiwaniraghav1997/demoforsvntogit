...
public class AuthorizationRFCService extends AbstractSAPJCOService {
	...
	public List<DeliveryOrShippingUnit> getOpenDeliveries(String d41id, String carrier, Date fromDate, Date toDate) throws Doc41ServiceException {
		Doc41Log.get().debug(this.getClass(), UserInSession.getCwid(), "getOpenDeliveries() - d41id=" + d41id + ", carrier=" + carrier + ".");
	...
	/**
	 * Check for the vendor (partner), if already a artwork document exist with the
	 * same material number. Current old version only checks, if any ArtWork
	 * document existing for vendor. Upload only allowed, if already first document
	 * existing (as follow up)
	 * 
	 * @param vendorNumber
	 *            the vondor/partner number
	 * @param materialNumber
	 *            the material number of their artwork document (new, not yet
	 *            checked by RFC but prepared TODO)
	 * @param sapDocType
	 *            = sapTypeId
	 * @return seems to be null if ok, else an error message (not 100% sure, if also
	 *         success message may be returned)
	...
	public String checkArtworkLayoutForVendor(String vendorNumber, String materialNumber, String sapDocType) throws Doc41ServiceException {
		Doc41Log.get().debug(this.getClass(), UserInSession.getCwid(), "checkArtworkForVendor() - vendorNumber=" + vendorNumber + ".");
	...
	/**
	 * Check PO and Material for Vendor. Now PO number is optional. Check without PO
	 * now checks, if there is any PO for Vendor related to specified material.
	 * SAP-Developer: IV_PONUMBER ist optional. Es wird nur noch RC = 0 zurück
	 * gegebenen wenn es PO Positionen mit nicht gesetztem Endlieferkennzeichen
	 * gibt.
	 * 
	 * @param vendorNumber
	 * @param poNumber
	 *            optional, no more specified in new version (if specified, auth rfc
	 *            is expected to only chech for this RFC)
	 * @param materialNumber
	 * @return
	 * @throws Doc41ServiceException
	 */
	public List<String> checkPOAndMaterialForVendor(String vendorNumber, String poNumber, String materialNumber, String customVersion, Date timeFrame) throws Doc41ServiceException {
		Doc41Log.get().debug(this.getClass(), UserInSession.getCwid(), "checkPOAndMaterialForVendor() - vendorNumber = " + vendorNumber + ".");
		List<Object> params = new ArrayList<Object>();
		params.add(vendorNumber);
		params.add(poNumber);
	...
	public List<InspectionLot> getInspectionLotsForVendorBatch(String vendor, String vendorBatch, String plant) throws Doc41ServiceException {
		Doc41Log.get().debug(this.getClass(), UserInSession.getCwid(), "getInspectionLotsForVendorBatch() - vendor=" + vendor + ", vendorBatch=" + vendorBatch + ", plant=" + plant + ".");
	...
	public List<QMBatchObject> getBatchObjectsForSupplier(String supplier, String plant, String material, String batch, String order) throws Doc41ServiceException {
		Doc41Log.get().debug(this.getClass(), UserInSession.getCwid(), "getBatchObjectsForSupplier() - supplier=" + supplier + ", plant=" + plant + ", material=" + material + ", batch=" + batch + ", order=" + order + ".");
	...
	public List<QMBatchObject> getBatchObjectsForCustomer(String customer, String delivery, String material, String batch, String country) throws Doc41ServiceException {
		Doc41Log.get().debug(this.getClass(), UserInSession.getCwid(), "getBatchObjectsForSupplier() - customer=" + customer + ", delivery=" + delivery + ", material=" + material + ", batch=" + batch + ", country=" + country + ".");
