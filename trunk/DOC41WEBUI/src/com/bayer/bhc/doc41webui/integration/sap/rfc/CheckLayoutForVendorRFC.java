...
public class CheckLayoutForVendorRFC extends AbstractDoc41RFC<Boolean>{
	...
	public void prepareCall(JCoFunction pFunction, List<?> pInputParms)
			throws SAPException {
		Doc41Log.get().debug(CheckLayoutForVendorRFC.class, null, "prepareCall():ENTRY");
        ...
        if (pFunction != null) {
            if (pInputParms != null) {
                ...
                throw new SAPException(
                        "CheckLayoutForVendorRFC pInputParms list is null", null);
            }
        } else {
            throw new SAPException(
                    "CheckLayoutForVendorRFC pFunction list is null", null);
        }
        Doc41Log.get().debug(CheckLayoutForVendorRFC.class, null, "prepareCall():EXIT");
	}
